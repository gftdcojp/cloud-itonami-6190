(ns telecom.operation
  "OperationActor -- one telecom-operator operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Telecom Advisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Telecom Access Governor
  ...
  One graph run = one telecom-operator operation (intake -> advise ->
  ...
  decision to a human operator (the telecom operator). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/provision-number`/`:actuation/suppress-billing-record`
  ALWAYS reach this node when the governor is clean -- see `telecom.
  phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [telecom.telecomadvisor :as telecomadvisor]
...
(defn build
  ...
  `telecom.store/Store`).
  opts:
    :advisor      -- a `telecom.telecomadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (telecomadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Telecom Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (telecomadvisor/-advise advisor store request)]
            {:proposal p :audit [(telecomadvisor/trace request p)]})))

      ;; Telecom Access Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
